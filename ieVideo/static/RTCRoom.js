...
RTCRoom = (function () {
    ...
    function init(object) {
        ...
        loginInfo = {
            ...
            'identifier': accountInfo.imuserID, //当前用户ID,必须是否字符串类型，选填
            'identifierNick': accountInfo.imuserID, //当前用户昵称，选填
    ...
    function getLivePusher() {
        ...
        livePusher.setRenderWndSize(0.1, 0.1);
        console.log("accountInfo.userID" + livePusher);
        if (!livePusher) {
            ...
            if (parentView) {
                ...
                //   'codebase="/sdk/LiteAVAX.cab#version=2,2,1,0"' +
                ...
                // var livePusher = document.getElementById(accountInfo.userID);
                var livePusher = document.getElementById('Pusher');
                // var livePusher = accountInfo.userID;
                livePusher.setRenderWndSize(0.1, 0.1);
            }
        }
        return livePusher;
    ...
    function loginIM(options) {
        ...
        console.log(roomInfo.isDestory)
        // 初始化设置参数
        webimhandler.init({
        ...
        var onGroupSystemNotifys = {
            // 群被解散(全员接收)
            "5": function (notify) {
                console.log('788' + notify)
                roomInfo.isDestory = true;
                event.onRoomClose(notify);
            },
        ...
        if (accountInfo.accountMode == 1) { //托管模式
            ...
            //sdk登录
            // alert('登录');
            console.log('449' + JSON.stringify(userInfo))
            webimhandler.sdkLogin(userInfo, listeners, others, 0, afterLoginIM, options);
        }
    }
    function afterLoginIM(options) {
        console.log("eeee" + JSON.stringify(options))
    ...
    function getPushers(options) {
        ...
        localStorage.setItem('accoutRoomID', roomInfo.roomID);
        //   console.log('664'+roomInfo.roomID)
        options = options || {};
        request({
            url: 'get_pushers',
    ...
    function getPushURL(options) {
        ...
        request({
            ...
            fail: function (ret) {
                console.log('获取推流地址fail：', ret.errCode);
                options.fail && options.fail({
                    errCode: ret.errCode || -3,
    ...
    function goHomeRouter(userID, roomID,cb) {
        ...
        if (RTC){
            RTC.quitRoom(userID, roomID, function(res) {
              if (cb) cb();
            });
        }
    }
    ...
    function startLocalPreview(object) {
        ...
        console.log("livePusher" + livePusher.startAudioCapture);
        ...
        livePusher.setVideoBitRate(200);
    ...
    function addRemoteView(object) {
        ...
        roomInfo.pushers.forEach(function (value) {
            if (value.userID == object.data.userId) {
                ...
                //     'codebase="/static/sdk/LiteAVAX.cab#version=2,2,1,0"\n' +
                //     'width=' + width.toString() + "\n"  +
                //     'height=' + height.toString() + "\n" +
                //     'events="True"></object>';
                //    parentView.innerHTML = html;
                if (parentView.innerHTML != '') {
    ...
    function setBitrateRange(minBitrate, maxBitrate) {
        ...
        livePusher.setVideoBitRateMxn(maxBitrate);
    }
...
function VideoBitRate() {
    ...
    pusher.setVideoBitRate(200);
    //pusher.setBitrateRange(200,500);
